...
function separateThousands(input: string): number[] {
    // Examples with input = '6486021351',
    const parts: string[] = reverse(input) // '1531206846'
        .match(/\d{1,3}/g) // ['153', '120', '684', '6']
        .reverse(); // ['6', '684', '120', '153']
    return parts.map(p => parseInt(reverse(p), 10)); // [6, 486, 21, 351]
}

function between(min: number, n: number, max: number): boolean {
    return n >= min && n <= max;
}
...
function getSeparatorName(index: number, arr: number[]): string {
...
function onClick() {
    const input = (<HTMLInputElement> document.getElementById('test')).value;
